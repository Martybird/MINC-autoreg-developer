...
/* 
   ...
   direction cosines for each axis, and NOT THE ABSOLUTE WORLD COORD! ..so not the absolute world..
...
void init_slices_lattice( VIO_Volume d2,
                          VIO_Volume m1, //mask data
                          VIO_Volume m2, 
                          Arg_Data *globals)
{
  ...
  int
      count1[VIO_MAX_DIMENSIONS], count2[VIO_MAX_DIMENSIONS];
  double 
        wstart1[VIO_MAX_DIMENSIONS], start1[VIO_MAX_DIMENSIONS],step1[VIO_MAX_DIMENSIONS];
  VectorR
      scaled_directions1[VIO_MAX_DIMENSIONS], directions1[VIO_MAX_DIMENSIONS];
  ...
  /* get start in volume order and wstart,count,step and directions in XYZ order */
  for (n = 0; n < globals->slices.num_slices; n++) {
    set_up_lattice(globals->slices.slice_array[n].slice_image, globals->step,
                 start1, wstart1, count1, step1, directions1);
    ...
    if (globals->flags.debug && globals->flags.verbose>1) {
      print ("in init_lattice: the new lattice for slice %s is:\n", globals->slices.slice_array[n].filename);
      print ("start = %8.2f %8.2f %8.2f %8.2f in vol order\n",start1[0],start1[1],start1[2],start1[3]);
      print ("wstart= %8.2f %8.2f %8.2f \n",wstart1[0],wstart1[1],wstart1[2]);
      print ("count = %8d %8d %8d \n",count1[0],count1[1],count1[2]);
      print ("step  = %8.2f %8.2f %8.2f \n",step1[0],step1[1],step1[2]);
    ...
    fill_Point( starting_position1, wstart1[0], wstart1[1], wstart1[2]); // aboslut world coord?
    ...
    for(r=0; r<count1[VIO_Y]; r++) {
      ...
      SCALE_VECTOR( vector_step, scaled_directions1[VIO_Y], r);
      ADD_POINT_VECTOR( row, starting_position1, vector_step );
  
      SCALE_POINT( col, row, 1.0); /* init first col position */
      for(c=0; c<count1[VIO_X]; c++) {
  
        convert_3D_world_to_voxel(globals->slices.slice_array[n].slice_image,
            Point_x(col), Point_y(col), Point_z(col), &tx, &ty, &tz);
        
        fill_Point( voxel, tx, ty, tz ); /* build the voxel POINT */
        
        if (point_not_masked(m1, Point_x(col), Point_y(col), Point_z(col))) {        // what exactly is m1 here?
  
          if (INTERPOLATE_TRUE_VALUE(globals->slices.slice_array[n].slice_image,
              &voxel, &true_value )) {
    ...
    /* set up the lattice on the source when not running NONLIN and lattice is
    not forced to target, otherwise, the lattice should be set up on the target*/
    if ( !(globals->trans_info.transform_type==TRANS_NONLIN) &&
          !(main_args.force_lattice==2) ) {
      
      globals->slices.slice_array[n].count[VIO_X] = max1_col - min1_col + 1;
      globals->slices.slice_array[n].count[VIO_Y] = max1_row - min1_row + 1;
      
      fill_Point( starting_position1, wstart1[0], wstart1[1], wstart1[2]); // so this is the world position?
      SCALE_VECTOR( vector_step, scaled_directions1[VIO_Y], min1_row);
      ADD_POINT_VECTOR( row, starting_position1, vector_step );
      SCALE_VECTOR( vector_step, scaled_directions1[VIO_X], min1_col);
      ADD_POINT_VECTOR( col, row, vector_step );
  
      globals->slices.slice_array[n].start[VIO_X] = Point_x(col);
      globals->slices.slice_array[n].start[VIO_Y] = Point_y(col);
      globals->slices.slice_array[n].start[VIO_Z] = Point_z(col);

  
      for(i=0; i<3; i++) {
  
        /* ensure that global step has same sign as data voxel */
        globals->slices.slice_array[n].step[i] = step1[i];  
        abs_step = fabs(globals->slices.slice_array[n].step[i]);
  
        Point_x(globals->slices.slice_array[n].directions[i]) = 
            Point_x(directions1[i]) * abs_step;
        Point_y(globals->slices.slice_array[n].directions[i]) = 
            Point_y(directions1[i]) * abs_step;
        Point_z(globals->slices.slice_array[n].directions[i]) = 
            Point_z(directions1[i]) * abs_step;
      }
    }
  }
