...
float fit_function(float *params) 
{
 ...
 (void)printf("out : %7.4f=%c %7.4f=%c %7.4f=%c   %7.4f=%c %7.4f=%c %7.4f=%c   %7.4f=%c %7.4f=%c %7.4f=%c \n",
    ...
    // this part needs to be adjusted for slice_to_volume xcorr registration
    
    r = (main_args.obj_function)(Gdata1,Gdata2,Gmask1,Gmask2,&main_args);
  }
  
  /*if (main_args.flags.debug) {
    parameters_to_vector(trans, rots, scale, shear, params, main_args.trans_info.weights);
                                              ...
                                              Arg_Data *globals)
{
  ...
  else if (globals->obj_function == slice_xcorr_objective) {
    /*EMPTY*/
  }
  else  {
  ...
  case TRANS_LSQ9: 
    ...
    //  globals->trans_info.scales[2] = 1.0; /* added by Yiming Xiao Dec 29, 2015 for calibration*/
    break;
  case TRANS_LSQ10: 
                                                     ...
                                                     Arg_Data *globals)
{
  ...
  case TRANS_LSQ9: 
    ...
    //globals->trans_info.scales[2] = 1.0; // added by Yiming Xiao DEc 29, 2015 for US cali
    break;
  case TRANS_LSQ10: 
    for(i=10; i<12; i++) globals->trans_info.weights[i] = 0.0;
    for(i=1; i<3; i++) {
