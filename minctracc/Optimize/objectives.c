                             ...
                             Arg_Data *globals)
{
  ...
  trans = get_linear_transform_ptr(vox_space->voxel_to_voxel_space); // this is voxel-to-voxel
...
float slice_xcorr_objective(VIO_Volume d1,
                            VIO_Volume d2,
                            VIO_Volume m1,
                            VIO_Volume m2, 
                            Arg_Data *globals)
{
  ...
  VIO_Real
      s1,s2,s3;                   /* to store the sums for f1,f2,f3 */
  float 
      result, xcorrelation;                                /* the result */
  int 
      count1,count2,count1_sum,count2_sum;
  ...
  //printf("Got to slice xcorr objective.\n");

  /* prepare counters for this objective function */
  result = xcorrelation = 0.0;
  count1_sum = count2_sum = 0;
  /* prepare data for the voxel-to-voxel space transformation (instead of the
  general but inefficient world-world computations. */

  vox_space = new_voxel_space_struct();
  
  for (i = 0; i < globals->slices.num_slices; i++) {

    get_into_slice_voxel_space(globals, vox_space, i, d2); // feed the information in, but how still kept d2 if not used?
  
    trans = get_linear_transform_ptr(vox_space->voxel_to_voxel_space);
  
  
    /* loop through all nodes of the lattice */
    // it appears these are all in voxel space instead of world space                                                                                                                            
    fill_Point( starting_position, vox_space->start[VIO_X], vox_space->start[VIO_Y], vox_space->start[VIO_Z]);  
    ...
    /* ---------- step through all rows of lattice ------------- */
    for(r=0; r<globals->slices.slice_array[i].count[VIO_X]; r++) {
      
      SCALE_VECTOR( vector_step, vox_space->directions[VIO_X], r);
      ADD_POINT_VECTOR( row, starting_position, vector_step );
      
      SCALE_POINT( col, row, 1.0); /* init first col position */ // why the need to multiply by 1?

      /* ---------- step through all cols of lattice ------------- */
      for(c=0; c<globals->slices.slice_array[i].count[VIO_Y]; c++) {
                
        /* use the voxel center closest to this lattice node. */
        // 
        fill_Point( voxel, VIO_ROUND(Point_x(col)), VIO_ROUND(Point_y(col)), VIO_ROUND(Point_z(col)) ); // voxel space, but on the lattice?


        if (voxel_point_not_masked(m1, Point_x(voxel), Point_y(voxel), Point_z(voxel))) {
          
          if (trilinear_interpolant( globals->slices.slice_array[i].slice_image,//nearest_neighbour_interpolant( globals->slices.slice_array[i].slice_image, // change by xiao, 2014 Dec
              &voxel, &value1 )) {
        ...
        ADD_POINT_VECTOR( col, col, vox_space->directions[VIO_Y] );
        
      } /* for c */
    } /* for r */    
    
    if (s2 > 0 && s3 > 0) {
      xcorrelation = 1.0 - s1 / (sqrt((double)s2)*sqrt((double)s3));
    } else {
      xcorrelation = 1.0;
    }
    result += xcorrelation;
    count1_sum += count1; count2_sum += count2;
    
    if (globals->flags.debug && globals->flags.verbose > 3) {
      print ("Slice %s: %7d %7d -> %10.8f\n",globals->slices.slice_array[i].filename,
             count1,count2,xcorrelation);
    }
  }
  
  result /= globals->slices.num_slices;
  
  if (globals->flags.debug) {
    print ("%7d %7d -> %10.8f\n", count1_sum,count2_sum,result);
  }
...
float src_gavg_objective(VIO_Volume d1,
                         VIO_Volume d2,
                         VIO_Volume m1,
                         VIO_Volume m2, 
                         Arg_Data *globals)
{
      ...
      for(c=0; c<globals->count[COL_IND]; c++) {
        ...
        if (voxel_point_not_masked(m1, Point_x(voxel), Point_y(voxel), Point_z(voxel))) {
          ...
          if (nearest_neighbour_interpolant( d1, &voxel, &value1 )) {
            ...
            } /* if voxel value from d1 is greater than threshold */
          } /* if voxel in d1 */
        } /* if point in mask volume one */
  ...
  if (globals->flags.debug) (void)print ("%7d %7d -> %10.8f\n",count1,count2,result);
  //print("translation %10.5f %10.5f %10.5f\n", trans->m[0][0], trans->m[1][0], trans->m[2][0]);
