...
int main ( int argc, char* argv[] )
{
  ...
  if (main_args.slices_to_volume) {
    switch (main_args.obj_function_type) {
      case XCORR:
        main_args.obj_function = slice_xcorr_objective;
        break;
      default:
        (void) fprintf(stderr, "Objective function not implemented for slices to volume registration\n");
        exit(EXIT_FAILURE);
    }
  } else  {
    switch (main_args.obj_function_type) {
  ...
  if (!parse_flag && main_args.slices_to_volume) {
    main_args.slices.num_slices = argc - 3;
    main_args.slices.slice_array = (Slice *)malloc(sizeof(Slice) * main_args.slices.num_slices);
    
    // Check if input files exist
    for(i = 0; i < main_args.slices.num_slices; i++){
      main_args.slices.slice_array[i].filename = argv[i + 1];
      if(!file_exists(main_args.slices.slice_array[i].filename)){
        fprintf(stderr, "%s: Couldn't find input file %s.\n\n", argv[0], main_args.slices.slice_array[i].filename);
        exit(EXIT_FAILURE);
      }
      //printf("input_file: %s\n", main_args.slices.slice_array[i].filename);
    }
    
    main_args.filenames.model = argv[argc - 2];
    main_args.filenames.output_trans = argv[argc - 1];
    
    if(!file_exists(main_args.filenames.model)){
      fprintf(stderr, "%s: Couldn't find model file %s.\n\n", argv[0], main_args.filenames.model);
      exit(EXIT_FAILURE);
    }
    ...
    // so this part deals with the matlab file for objective function
    //************************************************
        ...
        if (main_args.obj_function == vr_objective) {
          ...
          if (main_args.obj_function == ssc_objective) {
            ...
            if (main_args.obj_function == mutual_information_objective || main_args.obj_function == normalized_mutual_information_objective ) {
              ...
              if (main_args.obj_function == slice_xcorr_objective) {
                print("slices to volume cross correlation (threshold = %f %f)\n",
                    main_args.threshold[0],main_args.threshold[1]);
              }
              else {
              print("unknown!\n");
  ...
  if (main_args.slices_to_volume) {
    for (i = 0; i < main_args.slices.num_slices; i++) {
      ...
      status = input_volume( main_args.slices.slice_array[i].filename, VIO_N_DIMENSIONS,
                             default_dim_names, NC_UNSPECIFIED, FALSE, 0.0, 0.0,
                             TRUE, &(main_args.slices.slice_array[i].slice_image), (minc_input_options *)NULL );

      if (status != VIO_OK) {
        print_error_and_line_num("Cannot input image '%s'",
                                 __FILE__, __LINE__, main_args.slices.slice_array[i].filename);
      } else if (main_args.flags.debug) {
        printf("Read in slice %s.\n", main_args.slices.slice_array[i].filename);
      }
      // temporary line to try the blur function
      //blur_volume(main_args.slices.slice_array[i].slice_image, 20.0, 20.0, 20.0, 2, 1);
    }
    //exit(0);
    
    // Hack: Read in the model file as the data to fool the rest of the code to proceed.
...
#include "measure_code.c"
  ...
  if (main_args.trans_info.transform_type != TRANS_PAT) {
    ...
    if (main_args.slices_to_volume) {
      init_slices_lattice(model, mask_data, mask_model, &main_args);
      
      if (main_args.flags.debug && main_args.flags.verbose > 1) {
        for (i = 0; i < main_args.slices.num_slices; i++) {
          print ("Slice %s:\n", main_args.slices.slice_array[i].filename);
          DEBUG_PRINT3 ( "Lattice step size  = %8.3f %8.3f %8.3f\n",
                         main_args.slices.slice_array[i].step[0],main_args.slices.slice_array[i].step[1],main_args.slices.slice_array[i].step[2]);
          DEBUG_PRINT3 ( "Lattice start      = %8.3f %8.3f %8.3f\n",
                         main_args.slices.slice_array[i].start[0],main_args.slices.slice_array[i].start[1],main_args.slices.slice_array[i].start[2]);
          DEBUG_PRINT3 ( "Lattice count      = %8d %8d %8d\n\n",
                         main_args.slices.slice_array[i].count[0],main_args.slices.slice_array[i].count[1],main_args.slices.slice_array[i].count[2]);
        }
      }
    } else {
      init_lattice( data, model, mask_data, mask_model, &main_args );
